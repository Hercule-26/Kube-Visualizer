...
<template>
  <div class="pointer-events-auto flex flex-col items-end gap-4">
    <div class="overflow-hidden rounded-lg shadow-sm backdrop-blur">
      <URadioGroup v-model="store.layoutMode" :items="MODES" orientation="horizontal" variant="table" indicator="hidden"
        color="info" size="lg" :ui="{
    ...
    <div class="flex flex-col overflow-hidden rounded-lg border border-default bg-default/90 shadow-sm backdrop-blur">
      <UTooltip v-for="control in VIEW_CONTROLS" :key="control.label" :text="control.label" :content="{ side: 'left' }">
        <UButton :icon="control.icon" color="neutral" variant="ghost" size="sm" class="rounded-none"
