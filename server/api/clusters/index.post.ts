...
export default defineEventHandler(async (event) => {
  ...
  if (!body.insecureSkipTlsVerify && !body.certificate.trim()) {
    throw createError({
      statusCode: 400,
      statusMessage:
  ...
  if (body.allowWrite && body.editableKinds.length === 0) {
    throw createError({
      statusCode: 400,
      statusMessage:
  ...
  try {
    await testClusterConnection(body)
  }
  catch (error) {
    throw createError({
      statusCode: 502,
  ...
  return {
    ...
    cluster: {
      ...
      allowPodDelete: body.allowPodDelete,
      editableKinds: body.editableKinds,
    },
  }
})